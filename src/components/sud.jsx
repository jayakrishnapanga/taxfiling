...
//       console.log(files)
//       const groupedFiles =  groupFilesBySubmissionId(files);
//       console.log("hey this is from sud")
  ...
  useEffect(() => {
    const fetchGroupedFiles = async () => {
      try {
        ...
        if (response.status === 200) {
          const { currentSubmissions ,totalPages} = response.data;
          setLoading(false);
          setGroupedFiles(currentSubmissions);
          setTP(totalPages)
          console.log("this is for pafge");
          console.log(totalPages)
          console.log('pages: ', tp)
